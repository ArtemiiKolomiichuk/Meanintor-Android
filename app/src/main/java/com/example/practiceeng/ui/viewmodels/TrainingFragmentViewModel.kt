...
class TrainingFragmentViewModel(val amount:Int, val types: Array<TestType>, val folder: UUID?) : ViewModel() {
    ...
    fun isEmpty():Boolean{
        return questionBank?.isEmpty() ?: true
    }
    fun size(): Int = _questionBank!!.size

    fun moveToNext(): Boolean {

        Log.d("ViewModel", "moveToNext")
        if (!hasNext())
            return false
        currentIndex = currentIndex + 1

        Log.d("ViewModel", "moved to Next")
        return true
    }
    fun pushCurrentToEnd() {
        Log.d("ViewModel", "pushCurrentToEnd")
